...
def card() :
    ...
    main_layout.addWidget(attempts, alignment=Qt.AlignmentFlag.AlignCenter)
    main_layout.addStretch(1)
...
def setQuestion(my_question, tanswer, fanswer1, fanswer2, fanswer3):
    ...
    new_questions_text =   " Кількість спроб: "+ str(provider.attempts) +  " Кількість правильних відповідей: " + str(provider.right_answers)
    attempts.setText(new_questions_text)
...
def check_answer() :
    ...
    if(true_answer.isChecked()) :
        ...
        provider.increase_answers()
    else :
        resault.setText("Не вірно")
        provider.increase_attempts()
